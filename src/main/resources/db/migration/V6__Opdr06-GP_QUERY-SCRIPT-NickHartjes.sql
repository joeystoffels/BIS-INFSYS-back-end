...
WHERE A.SPEL_OF_CONSOLE = 'CONSOLE' AND HO.STARTDATUM <= GETDATE() AND HO.EINDDATUM >= GETDATE() AND
      HO.HUURSTATUS = 'VERHUURD';
...
FROM HUUROVEREENKOMST AS HO
  ...
  INNER JOIN ARTIKEL AS A ON AV.BARCODE = A.BARCODE
WHERE
  (MONTH(HO.STARTDATUM) = 5 AND YEAR(HO.STARTDATUM) = 2015) OR (MONTH(HO.EINDDATUM) = 5 AND YEAR(HO.EINDDATUM) = 2015);
