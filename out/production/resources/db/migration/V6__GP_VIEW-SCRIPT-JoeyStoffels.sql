...
SELECT 
			...
			A.[TYPE],
			A.[UITGEVER],
			B.[AANTAL_KEER_VERHUURD]
FROM 
			ARTIKEL A,
			(SELECT TOP 1 AV.[BARCODE], COUNT(AV.[BARCODE]) AS 'AANTAL_KEER_VERHUURD'
			FROM ARTIKELENVERHUUR AV 
			INNER JOIN ARTIKEL A ON A.[BARCODE] = AV.[BARCODE]
			WHERE A.SPEL_OF_CONSOLE = 'SPEL'
			GROUP BY AV.[BARCODE]
			ORDER BY 'AANTAL_KEER_VERHUURD' DESC) AS B
WHERE 
			A.[BARCODE] = B.[BARCODE];
GO
...
FROM
			HUUROVEREENKOMST HO,
			(SELECT TOP 1 (CONVERT(INT, HO.[EINDDATUM] - HO.[STARTDATUM]) * A.[PRIJS_PER_D]) AS 'BEDRAG'
			FROM ARTIKEL A
			INNER JOIN ARTIKELENVERHUUR AV ON A.[BARCODE] = AV.[BARCODE]
			INNER JOIN HUUROVEREENKOMST HO ON AV.[EMAILADRES] = HO.[EMAILADRES] AND AV.[STARTDATUM] = HO.[STARTDATUM]
			ORDER BY 'BEDRAG' DESC) AS B;
GO
