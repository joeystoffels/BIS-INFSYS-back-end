...
SELECT 
    ...
    SUM(CASE WHEN HO.HUURSTATUS = 'VERHUURD' THEN DATEDIFF(d, HO.STARTDATUM, HO.EINDDATUM) * A.PRIJS_PER_D ELSE 0 END) AS 'omzet'
FROM ARTIKEL AS A
...
WHERE (YEAR(AV.DATUM) = 2017 AND MONTH(AV.DATUM) = 1) 
OR 
(HO.HUURSTATUS = 'VERHUURD' AND YEAR(AVR.STARTDATUM) = 2017 AND MONTH(AVR.STARTDATUM) = 1);
GO
